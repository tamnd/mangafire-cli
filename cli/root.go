// Package cli builds the mangafire command tree on top of the mangafire library.
package cli
...
type App struct {
	client   *mangafire.Client
	cfg      mangafire.Config
	output   string
	fields   []string
	noHeader bool
	template string
...
func Root() *cobra.Command {
	app := &App{cfg: mangafire.DefaultConfig()}

	root := &cobra.Command{
		Use:           "mangafire",
		Short:         "Browse the MangaFire manga catalog",
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.IntVarP(&app.limit, "limit", "n", 0, "limit number of records (0 = all on fetched pages)")
...
func (a *App) setup() error {
	...
	a.client = mangafire.NewClient(a.cfg)
	return nil
}
