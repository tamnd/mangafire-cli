...
describes what users get. Pick a format with `-o`, or let mangafire choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mangafire <command> -o table   # aligned columns for reading
mangafire <command> -o jsonl   # one JSON object per line, for piping
mangafire <command> -o json    # a single JSON array
mangafire <command> -o csv     # spreadsheet friendly
mangafire <command> -o tsv     # tab-separated
mangafire <command> -o url     # just the URL column
mangafire <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mangafire <command> --fields id,title,url
```
...
```bash
mangafire <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mangafire <command>            # a table, because this is a terminal
mangafire <command> | wc -l    # JSONL, because this is a pipe
```
